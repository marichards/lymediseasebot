            ...
            if not re.search('^RT',status.text):
                ...
                if status.author.screen_name not in author_list:
                    ...
                    try:
                        ...
                        .format(v0 = status.author.screen_name, v1 = status.author.name.replace("'"," "),\
                        v2 = status.author.followers_count, v3 = status.author.friends_count, v4 = status.author.statuses_count))
                        self.conn.commit()
                    except:
                        print("Error: {} and {}"\
                        .format(status.author.screen_name.encode("utf8"),status.author.name.encode("utf8")))
                    #test = "INSERT INTO authors ('author','followers') VALUES ('{v1}', {v2})".format(v1 = status.author.screen_name, v2 = 1)
                    #print(test)
                else:
